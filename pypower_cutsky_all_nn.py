...
for n in np.arange(args.phstart,args.phend+1): # AJRM added start en stop
    ...
    if not os.path.exists(mock_dir+'results_pypower_'+str(nmesh)+'_corrected/Pk_'+filename_orig+'_zmin'+str(zmin)+'_zmax'+str(zmax)+'.npy'):
        ...
        result=CatalogFFTPower(data_positions1=data_positions,data_weights1=data_weights,randoms_positions1=randoms_positions,
        ...
        result.save(mock_dir+'results_pypower_'+str(nmesh)+'_corrected/Pk_'+filename_orig+'_zmin'+str(zmin)+'_zmax'+str(zmax)+'.npy')