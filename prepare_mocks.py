...
cont_dir = os.path.join(scratch,'test_sysnet','contaminated_mocks')
...
for ph in phs:
    ...
    mock_fn = os.path.join(cont_dir, f'cutsky_LRG_z0.800_AbacusSummit_base_c000_ph{ph:003d}.fits')
    mock_cont = Table.read(mock_fn)
    print(f"preparing {mock_fn}")

    # prepare data for NN
    prep_data = prepare_LRGmock_data(mock_cont, randoms, systematics, hp_order=hp_order, zmin=zmin, zmax=zmax, nside=nside, columns=maps_dr9)
    ...
    fn = os.path.join(prep_data_dir, f'cutsky_LRG_z0.800_AbacusSummit_base_c000_ph{ph:003d}.fits')
